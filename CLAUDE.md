...
## 열려있는 항목
- **[교체 대기] 기능 02번 블록 영상** — 글은 '현금흐름표 자동 생성'인데
  영상은 `demo-02-queries.mp4`(C1 재무제표 분석 → E2 특이사항 증빙)라 불일치.
  C3 현금흐름표 클립(10~15초, 1400px, 무음)을 받으면 아래를 함께 교체할 것:
  `data-src` / `poster` / `aria-label` (현재 aria-label 은 영상 내용 기준으로 정확히 유지 중)
